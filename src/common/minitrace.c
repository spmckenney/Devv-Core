...
// Will probably need to move arguments to a second buffer to support more than
// one.
...
//	 pthread basics
#ifdef _WIN32
static int get_cur_thread_id() { return (int)GetCurrentThreadId(); }
...
static inline int get_cur_thread_id() { return (int)(intptr_t)pthread_self(); }

#if defined(BLACKBERRY)
double mtr_time_s() {
  struct timespec time;
  clock_gettime(CLOCK_MONOTONIC,
                &time);  // Linux must use CLOCK_MONOTONIC_RAW due to time warps
...
static void termination_handler(int signum) {
  (void) signum; // warning: unused parameter 'signum' [-Wunused-parameter]
...
void mtr_init(const char *json_file) {
  ...
  return;
#endif
  buffer = (raw_event_t *)malloc(INTERNAL_MINITRACE_BUFFER_SIZE *
                                 sizeof(raw_event_t));
  is_tracing = 1;
  count = 0;
...
const char *mtr_pool_string(const char *str) {
  ...
  for (i = 0; i < STRING_POOL_SIZE; i++) {
    if (!str_pool[i]) {
      ...
      if (!strcmp(str, str_pool[i])) return str_pool[i];
    }
  }
...
void mtr_flush() {
  ...
  char arg_buf[256];
  char id_buf[256];
  // We have to lock while flushing. So we really should avoid flushing as much
  // as possible.

  pthread_mutex_lock(&mutex);
  int old_tracing = is_tracing;
  is_tracing = 0;  // Stop logging even if using interlocked increments instead
                   // of the mutex. Can cause data loss.
  ...
  for (i = 0; i < count; i++) {
    ...
    switch (raw->arg_type) {
      case MTR_ARG_TYPE_INT:
        snprintf(arg_buf, ARRAY_SIZE(arg_buf), "\"%s\":%i", raw->arg_name,
                 raw->a_int);
        break;
    ...
    if (raw->id) {
      switch (raw->ph) {
        ...
        case 'F':
          ...
          snprintf(id_buf, ARRAY_SIZE(id_buf), ",\"id\":\"0x%08x\"",
                   (uint32_t)(uintptr_t)raw->id);
          break;
        case 'X':
          snprintf(id_buf, ARRAY_SIZE(id_buf), ",\"dur\":%i",
                   (int)raw->a_double);
          break;
      }
    } else {
    ...
    len = snprintf(linebuf, ARRAY_SIZE(linebuf),
                   ...
                   first_line ? "" : ",\n", cat, raw->pid, raw->tid,
                   raw->ts - time_offset, raw->ph, raw->name, arg_buf, id_buf);
...
void internal_mtr_raw_event(const char *category, const char *name, char ph,
  ...
  return;
#endif
  if (!is_tracing || count >= INTERNAL_MINITRACE_BUFFER_SIZE) return;
