...
//
// Ultra-light dependency free library for performance tracing C/C++
// applications.
...
// overhead.
// Preferably, set this flag in your build system. If you can't just uncomment
// this line.
#define MTR_ENABLED
...
// Returns the current time in seconds. Used internally by Minitrace. No
// caching.
double mtr_time_s();
...
// If str is semi dynamic, store it permanently in a small pool so we don't need
// to malloc it.
...
void internal_mtr_raw_event(const char *category, const char *name, char ph,
                            void *id);
void internal_mtr_raw_event_arg(const char *category, const char *name, char ph,
                                void *id, mtr_arg_type arg_type,
                                const char *arg_name, void *arg_value);

#ifdef MTR_ENABLED

// c - category. Can be filtered by in trace viewer (or at least that's the
// intention).
//     A good use is to pass __FILE__, there are macros further below that will
//     do it for you.
// n - name. Pass __FUNCTION__ in most cases, unless you are marking up parts of
// one.
...
// Async events. Can span threads. ID identifies which events to connect in the
// view.
#define MTR_START(c, n, id) internal_mtr_raw_event(c, n, 'S', (void *)(id))
#define MTR_STEP(c, n, id, step)                                \
  internal_mtr_raw_event_arg(c, n, 'T', (void *)(id),           \
                             MTR_ARG_TYPE_STRING_CONST, "step", \
                             (void *)(step))
#define MTR_FINISH(c, n, id) internal_mtr_raw_event(c, n, 'F', (void *)(id))

// Flow events. Like async events, but displayed in a more fancy way in the
// viewer.
...
// The same macros, but with a single named argument which shows up as metadata
// in the viewer.
// _I for int.
// _C is for a const string arg.
// _S will copy the string, freeing on flush (expensive but sometimes
// necessary).
...
#define MTR_BEGIN_C(c, n, aname, astrval)                                    \
  internal_mtr_raw_event_arg(c, n, 'B', 0, MTR_ARG_TYPE_STRING_CONST, aname, \
                             (void *)(astrval))
#define MTR_END_C(c, n, aname, astrval)                                      \
  internal_mtr_raw_event_arg(c, n, 'E', 0, MTR_ARG_TYPE_STRING_CONST, aname, \
                             (void *)(astrval))
#define MTR_SCOPE_C(c, n, aname, astrval)                                 \
  MTRScopedTraceArg ____mtr_scope(c, n, MTR_ARG_TYPE_STRING_CONST, aname, \
                                  (void *)(astrval))

#define MTR_BEGIN_S(c, n, aname, astrval)                                   \
  internal_mtr_raw_event_arg(c, n, 'B', 0, MTR_ARG_TYPE_STRING_COPY, aname, \
                             (void *)(astrval))
#define MTR_END_S(c, n, aname, astrval)                                     \
  internal_mtr_raw_event_arg(c, n, 'E', 0, MTR_ARG_TYPE_STRING_COPY, aname, \
                             (void *)(astrval))
#define MTR_SCOPE_S(c, n, aname, astrval)                                \
  MTRScopedTraceArg ____mtr_scope(c, n, MTR_ARG_TYPE_STRING_COPY, aname, \
                                  (void *)(astrval))

#define MTR_BEGIN_I(c, n, aname, aintval)                           \
  internal_mtr_raw_event_arg(c, n, 'B', 0, MTR_ARG_TYPE_INT, aname, \
                             (void *)(intptr_t)(aintval))
#define MTR_END_I(c, n, aname, aintval)                             \
  internal_mtr_raw_event_arg(c, n, 'E', 0, MTR_ARG_TYPE_INT, aname, \
                             (void *)(intptr_t)(aintval))
#define MTR_SCOPE_I(c, n, aname, aintval)                        \
  MTRScopedTraceArg ____mtr_scope(c, n, MTR_ARG_TYPE_INT, aname, \
                                  (void *)(intptr_t)(aintval))
...
#define MTR_INSTANT_C(c, n, aname, astrval)                              \
  internal_mtr_raw_event(c, n, 'I', 0, MTR_ARG_TYPE_STRING_CONST, aname, \
                         (void *)(astrval))
#define MTR_INSTANT_I(c, n, aname, aintval)                     \
  internal_mtr_raw_event(c, n, 'I', 0, MTR_ARG_TYPE_INT, aname, \
                         (void *)(aintval))

// Counters (can't do multi-value counters yet)
#define MTR_COUNTER(c, n, val)                                  \
  internal_mtr_raw_event_arg(c, n, 'C', 0, MTR_ARG_TYPE_INT, n, \
                             (void *)(intptr_t)(val))
...
#define MTR_META_PROCESS_NAME(n)                         \
  internal_mtr_raw_event_arg("", "process_name", 'M', 0, \
                             MTR_ARG_TYPE_STRING_COPY, "name", (void *)(n))
#define MTR_META_THREAD_NAME(n)                         \
  internal_mtr_raw_event_arg("", "thread_name", 'M', 0, \
                             MTR_ARG_TYPE_STRING_COPY, "name", (void *)(n))
#define MTR_META_THREAD_SORT_INDEX(i)                         \
  internal_mtr_raw_event_arg("", "thread_sort_index", 'M', 0, \
                             MTR_ARG_TYPE_INT, "sort_index", (void *)(i))
...
#ifdef MTR_ENABLED
// These are optimized to use X events (combined B and E). Much easier to do in
// C++ than in C.
...
class MTRScopedTraceArg {
 public:
  MTRScopedTraceArg(const char *category, const char *name,
                    mtr_arg_type arg_type, const char *arg_name,
                    void *arg_value)
      : category_(category), name_(name) {
    internal_mtr_raw_event_arg(category, name, 'B', 0, arg_type, arg_name,
                               arg_value);
  }
  ~MTRScopedTraceArg() { internal_mtr_raw_event(category_, name_, 'E', 0); }
