...
//@note make sure utx_pool.HasProposal() == false before calling
DevcashMessageUniquePtr CreateNextProposal(const KeyRing& keys,
  ...
  if (utx_pool.HasPendingTransactions()) {
	  ...
	  LOG_WARNING << "WARN: No pending transactions! Pool is empty!";
  }
...
void DevcashController::ValidatorCallback(DevcashMessageUniquePtr ptr) {
  CASH_TRY {
    ...
    if (ptr->message_type == TRANSACTION_ANNOUNCEMENT) {
      ...
      } //else: Already a pending proposal
    } else { //Not a Transaction Announcement message
...
bool HandleFinalBlock(DevcashMessageUniquePtr ptr,
  ...
  if (!utx_pool.HasPendingTransactions()) {
	...
	if (!utx_pool.HasProposal()) {
      ...
      LOG_WARN << "WARN: Proposal already exists when receiving remote final block!";
    }
  }
  return sent_message;
}
